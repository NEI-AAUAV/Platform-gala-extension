...
const SOCIAL_LINKS = [
  { Icon: InstagramIcon, href: "https://www.instagram.com/nei.aauav/", label: "Instagram" },
  { Icon: LinkedInIcon, href: "https://www.linkedin.com/company/nei-aauav/", label: "LinkedIn" },
  { Icon: FacebookIcon, href: "https://www.facebook.com/NEIAAUAv", label: "Facebook" },
      ...
      <div className="mx-auto max-w-screen-xl px-4 py-14 md:px-8">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="flex flex-col items-start gap-4">
            <Link to="/" className="flex items-center font-gala group">
              <img 
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3">
              {NAV_LINKS.map(({ href, label, internal }) => (
                <li key={label}>
                  {internal ? (
                    <Link to={href} className="font-gala text-xs text-white/40 transition-colors hover:text-light-gold">
                      {label}
                    </Link>
                  ) : (
                    <a href={href} className="font-gala text-xs text-white/40 transition-colors hover:text-light-gold">
                      {label}
                    </a>
                  )}
                </li>
            ...
            <div className="mt-6 flex gap-3">
              {SOCIAL_LINKS.map(({ label, href, Icon }) => (
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label={label}
                  className="flex h-8 w-8 items-center justify-center border border-white/15 text-white/40 transition-all hover:border-light-gold/60 hover:text-light-gold"
                >
                  <Icon />
                </a>
              ))}
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-white/[0.06] pt-6 sm:flex-row">
          <p className="font-gala text-[0.65rem] uppercase tracking-wider text-white/25">
            &copy; {new Date().getFullYear()} NEI-AAUAv. Todos os direitos reservados.
          </p>
          <p className="font-gala text-[0.65rem] uppercase tracking-wider text-white/25">
