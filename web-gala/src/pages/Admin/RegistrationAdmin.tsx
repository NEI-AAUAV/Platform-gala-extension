...
import {
  Field, TextInput, NumberInput, DateTimeInput,
  Toggle, Section, StringListEditor, INPUT_CLS,
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={() => { resetConfig(); setSaved(true); setTimeout(() => setSaved(false), 2000); }}
              className="flex items-center gap-2 rounded-full border border-white/15 px-4 py-1.5 text-xs text-white/40 transition hover:border-white/30 hover:text-white/70"
            >
        ...
        <div ref={ref("dates")}>
          <Section title="1. Datas do Sistema" defaultOpen>
            <p className="text-xs text-white/40">
              Controla quando cada fase está aberta. As datas das mesas são geridas no separador{" "}
        ...
        <div ref={ref("phased")}>
          <Section title="8. Pagamento Faseado (Dual-Proof)">
            <Toggle enabled={config.phasedPaymentEnabled} onChange={(v) => save({ phasedPaymentEnabled: v })} label="Ativar Pagamento em 2 Fases" />
        ...
        <div ref={ref("security")}>
          <Section title="10. Segurança & Regras de Upload">
            <p className="text-xs text-white/40">
              Tamanho máximo: <span className="font-semibold text-white/60">10 MB</span> por comprovativo. Formatos aceites:{" "}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {SYSTEM_DATE_FIELDS.map(([field, label]) => (
          <Field key={field} label={label}>
            <DateTimeInput value={edits[field] ?? ""} onChange={(v) => handleChange(field, v)} />
          </Field>
        ))}
      </div>
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <Field label="Limite de inscrições">
        <div className="flex gap-2">
          <input type="number" min={1} value={maxRegistrations || ""} onChange={(e) => setMaxRegistrations(Number(e.target.value))} className={`flex-1 ${INPUT_CLS}`} />
          <button type="button" onClick={() => saveField("maxRegistrations", maxRegistrations)} className="shrink-0 rounded-lg border border-dark-gold/40 px-3 py-1.5 text-xs font-semibold text-dark-gold transition hover:bg-dark-gold/10">Guardar</button>
        </div>
      </Field>
      <Field label="Lugares de autocarro">
        <div className="flex gap-2">
          <input type="number" min={0} value={maxBusSeats || ""} onChange={(e) => setMaxBusSeats(Number(e.target.value))} className={`flex-1 ${INPUT_CLS}`} />
          <button type="button" onClick={() => saveField("maxBusSeats", maxBusSeats)} className="shrink-0 rounded-lg border border-dark-gold/40 px-3 py-1.5 text-xs font-semibold text-dark-gold transition hover:bg-dark-gold/10">Guardar</button>
        </div>
      </Field>
    </div>
  );
