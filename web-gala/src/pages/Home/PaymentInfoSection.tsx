...
function PriceCard({
  ...
  return (
    ...
    >
      ...
      <FontAwesomeIcon icon={icon} className="mb-4 text-2xl text-light-gold/50" />
      <p className="font-gala text-xs font-bold uppercase tracking-[0.3em] text-white/40">{label}</p>
      <p className="mt-2 font-gala text-5xl font-black text-light-gold">{amount}€</p>
...
function DeadlineCard({ index, registrationConfig }: { index: number; registrationConfig: RegistrationConfig }) {
  return (
    <motion.div
      ...
      viewport={{ once: true, margin: "-60px" }}
      transition={{ delay: index * 0.1, duration: 0.6 }}
      className="flex flex-col gap-5 rounded-2xl border border-white/8 bg-white/3 p-8"
    >
...
function InfoLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col">
      ...
      <span className="font-gala text-sm font-semibold text-white/70">{value || "A anunciar"}</span>
    </div>
  );
}
