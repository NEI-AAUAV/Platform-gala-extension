...
import { faCalendarDay, faClock, faLocationDot, faUserTie } from "@fortawesome/free-solid-svg-icons";
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {infoBoxes.map((info) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.1 * infoBoxes.indexOf(info), duration: 0.6 }}
            ...
            >
              ...
              <h3 className="font-gala text-lg font-bold text-white/90 uppercase tracking-widest">{info.label}</h3>
              <p className="mt-2 font-gala text-base text-neutral-300 ">{info.value}</p>
            </motion.div>
          ))}
        </div>
      </div>
