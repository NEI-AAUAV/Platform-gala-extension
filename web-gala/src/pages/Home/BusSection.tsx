...
import { faBus, faClock, faLocationDot } from "@fortawesome/free-solid-svg-icons";
...
function ScheduleInfo({ busConfig }: { busConfig: BusScheduleConfig }) {
  return (
    <motion.div
      ...
      className="flex flex-col gap-6 rounded-2xl border border-white/8 bg-white/3 p-8 lg:col-span-1"
    >
...
function ScheduleLine({ icon, label, value }: { icon: typeof faBus; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        <p className="font-gala text-[0.6rem] font-bold uppercase tracking-widest text-white/35">{label}</p>
...
function BusList({ buses }: { buses: BusScheduleConfig["buses"] }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col gap-4 rounded-2xl border border-white/8 bg-white/3 p-8 lg:col-span-2"
    >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {buses.map((bus, i) => (
          <motion.div
            ...
            className="flex items-center justify-between rounded-xl border border-white/6 bg-white/3 px-5 py-4"
          >
            <div className="flex items-center gap-3">
              <FontAwesomeIcon icon={faBus} className="text-light-gold/40" />
              <span className="font-gala text-sm font-bold text-white/80">{bus.name}</span>
            </div>
