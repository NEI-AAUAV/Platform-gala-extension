...
export default function Nominate() {
  ...
  const handleSubmitAll = async () => {
    const itemsToSubmit = nominationCategories
      .filter(cat => {
        ...
        return (!cat.already_nominated || forceEdit[cat._id]) && names.length > 0;
      })
      .map(cat => ({
        category_id: cat._id,
    ...
    itemsToSubmit.forEach(item => {
      ...
      if (item.names.length < cat.min_nominees) {
        newErrors[item.category_id] = `Precisas de adicionar pelo menos ${cat.min_nominees} ${cat.min_nominees === 1 ? "nome" : "nomes"}.`;
        hasErrors = true;
      }
    });
    ...
    try {
      ...
      if (result.status === "success" || result.status === "partial_success") {
        ...
        const successfulCatIds = itemsToSubmit
          .filter(item => !result.errors?.some(e => e.category_id === item.category_id))
          .map(item => item.category_id);
          
        successfulCatIds.forEach(id => {
          const finalName = [...nominations[id]].sort().join(" & ");
  ...
  const hasChanges = nominationCategories.some(cat => 
    (!cat.already_nominated || forceEdit[cat._id]) && (nominations[cat._id]?.length ?? 0) > 0
  );
    ...
    <div className="px-4 pb-24 pt-16 sm:px-8 sm:pt-20">
      <div className="mx-auto max-w-5xl text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl font-gala text-sm text-white/55 sm:text-base">
          Indica colegas para cada categoria. Podes preencher várias categorias e submeter tudo no final.
        </p>
      </div>
          ...
          <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
            <div className="hidden sm:block">
              <p className="font-gala text-xs text-white/40">
                {hasChanges ? "Tens nomeações pendentes para submeter." : "Preenche as categorias acima."}
              </p>
            </div>
            <button
              type="button"
